...
func TestSession_InitSession(t *testing.T) {
	...
	if sessKind != reflect.ValueOf(sm).Kind() {
		t.Error("Wrong kind returned testing cooking session. Expected", reflect.ValueOf(sm).Kind(), " and got", sessKind)
	}

	if sessType != reflect.ValueOf(sm).Type() {
		t.Error("Wrong type returned testing cooking session. Expected", reflect.ValueOf(sm).Type(), " and got", sessType)
	}
