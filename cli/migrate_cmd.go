...
var migrateVersionCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		version, dirty, err := b.MigrateVersion(dsn)
		if err != nil {
			exitGracefully(err)
		}
...
func doMigrate(caseToCheck string, steps string) error {
	...
	switch caseToCheck {
	case "up":
		if err := b.RunMigrations(dsn); err != nil {
			return err
		}
	case "down":
		if steps == "all" {
			return b.MigrateDown(dsn, -1)
		}
		...
		return b.MigrateDown(dsn, stepsInt)
	case "reset":
		if err := b.MigrateReset(dsn); err != nil {
			return err
		}
	default:
